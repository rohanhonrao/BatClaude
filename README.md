...
| **Alcove** | A perfume collection on two shelves — **Collected** and **Coveted** — shown as bottles, not a list. Add one with just a name, a house and a shelf — bottles in the shared library arrive with their own photo, cut off its catalogue background and set on the app's own black so the bottle floats rather than sitting in a white box. Shoot your own if you prefer; yours always wins over the catalogue shot, and removing it brings the catalogue one back. Wear, original-or-dupe, notes and where to buy are meant to arrive from research. Records sellers with price and *your* authenticity judgement, and gathers decant sources in one place. Bottles from the houses you collect — Lattafa, Afnan, Rasasi and the rest — already carry a dupe confirmation, a price and a short review summary from the shared library, so they fill themselves in. For anything that isn't there, **Ask about this one** opens a pre-filled request on GitHub: one tap, nothing to copy, and the answer arrives as a library update. Stays on your phone — not shared. |
